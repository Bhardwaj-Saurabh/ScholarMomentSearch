...
|---|-----------|------|-------|
| 55 | Production warm path | `fly.toml`, `src/app.py`, `src/rag/rerank.py`, `src/rag/embeddings.py` | Fly Upstash Redis (`REDIS_URL` secret) activates the component 19-21 caches + rate limiting that are silently no-op in prod today; `min_machines_running = 1` for the api group (kills machine cold starts polluting p95 and the observed ~70s first query); startup warm-up of the two lazy in-process models (`rerank._model()`, `embeddings._sparse_model()` — 5.7s+ cold today), each wrapped so a failed warm never blocks boot. |
| 56 | Accept-path fast 202 for documents | `src/api/admin.py`, `src/jobs.py`, `src/db.py`, `src/reconciler.py`, `src/ingest/doc_pipeline.py` | Registration handler does ONE insert then returns 202; `enqueue_document` + `set_document_flow_run_id` move to a Starlette background task (the response never used flow_run_id). Reconciler is the crash-safety net (a stale `pending` row with NULL flow_run_id — accepted but never dispatched — is re-enqueued; every other active status with NULL keeps the component-10 seeded-row skip). DB round-trip diet: pool goes `autocommit=True` (each single-statement helper drops BEGIN/COMMIT, 2 of every 4 RTs); the per-checkout `check=check_connection` ping is KEPT (amended 2026-07-31 during build: dropping it risks sporadic 500s against the frozen `error_rate_max_pct` gate to save one ~60ms round trip — the conservative half of the diet was chosen); the one genuinely multi-statement block (`wfq_claim`) gets explicit `conn.transaction()`. `jobs.py` reuses one long-lived Prefect sync client (was: fresh client + transatlantic TLS handshake per dispatch). Live-found follow-up, same component: the deferral widened the 202-then-DELETE race (bench probes delete immediately), so the background task re-checks the row before enqueueing and `t_fetch` treats a missing manifest row as a clean no-op instead of a twice-retried failure. |
| 57 | Ingest throughput package | `src/reconciler.py`, `src/ingest/doc_pipeline.py`, `src/db.py`, `src/llm.py`, `src/rag/vector_store.py`, `fly.toml` | Reconciler liveness fix (refined 2026-07-31 during build — blanket "SCHEDULED/PENDING/RUNNING are alive" would break the resilience gate, since killed-mid-launch leaves PENDING and killed-mid-execution leaves RUNNING): a minimal worker heartbeat (`src/liveness.py`, TTL'd Redis key written by the worker — deliberately NOT the full component-35 remit) becomes the discriminator; SCHEDULED + fresh heartbeat = healthy backlog, skip; PENDING/RUNNING keep restarting regardless; no Redis/heartbeat/any error = today's restart-happy behavior, failing toward over-admission, never stranding. `WORKER_CONCURRENCY` 2→6 and `PREFECT_RUNNER_POLL_FREQUENCY` 10s→2s in the worker group. Caption path: module-cached OpenAI client + bounded ThreadPoolExecutor over `t_caption`'s serial loop (component 21's Redis caption cache goes live via 55). Per-doc Postgres diet: merge adjacent status writes, reuse the already-fetched row instead of a second `get_document`. `ensure_text_collection()` once per process (module flag) instead of ~5 no-op Qdrant calls per flow run. |
