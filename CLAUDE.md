...
| 51 | Seeding vector-integrity verification | unit: `status='indexed'` + zero vectors ⇒ re-selected for seeding; `status='indexed'` + vectors present ⇒ skipped; a Qdrant error during the check fails open toward RE-SEEDING, never toward trusting a possibly-stale status; live: delete a seeded source's vectors, leave its row `indexed`, restart seeding, confirm it rebuilds |
